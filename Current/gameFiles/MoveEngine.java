...
/**
 * This is the engine that calculates all collisions and movement in the game. It is closely tied with gameEngine where <br>
 * the two classes are dependant on each other. The collision methods and moveEnts ()were coded by Hock-Chuan Chua and modified to fit the program.
 * The methods that apply forces to the ball were written by Mike, also modified.
 * @author Jason P'ng
...
public class MoveEngine
{
   ...
   */
    public static final float GRAVITY = 0.45f;
    /**
   * Constant for the drag force.
   */
  public static final float DRAG = 0.00001f;
  /**
   * Constant for terminal velocity.
   */
  public static final int MAX_SPEED = 500;
  /**
   * GameEngine used to access objects.
   */
  private static GameEngine main;
  /**
   * list of constant forces to apply every millisecond.
   */
  private ArrayList<Accel> constForces = new ArrayList<Accel>();
  /**
   * Temporary response used to find the earliest collision.
   */
  private static CollisionResponse tempResponse = new CollisionResponse();
  
  /**
   * Constructor which sets the GameEngine and sets up the constant forces.
   */
  ...
  /**
   * Main method used to move all objects.
   */
  public void run()
  {
    applyConstForces ();
  ...
  /**
   * Adds the force of gravity to the constant forces. Other forces may be added.
   */
  ...
  /**
   * Applies all constant forces to all balls in the game engine. <br>
   * for (Accel a : constForces) - traverses through the constant forces array to add them together. <br>
   * for (Ball s : main.living) - travers through the list of balls to apply forces to all of them.
   * 
   * @param xAccel total acceleration along x axis.
   * @param yAccel total acceleration along y axis.
   */
  ...
  /**
   * Applies the forces acting on each of the balls in the game engine. <br>
   * for (int i = 0; i < main.living.size(); i++) - Traverse through the array of balls to apply forces on all of them.
   * 
   * @param Ball s the ball being acted upon
   * @param Accel theAccel the total force being acted on the ball.
   * @param float vx the new x velocity for the ball.
   * @param float vy the new y velocity for the ball.
   */
  private synchronized void sumForces()
  {
    ...
    /**
   * Main method for determining collisions and locations of the ball. <br>
   * Written by Hock-Chuan Chau modified by Jason P'ng <br>
   * do while (time > 1e-2f) - loop to calculate all collisions within a millisecond of time. The loop ends when a millisecond has passed. <br>
   * for loops are used to traverse through arrays of objects. The ball checks to see if it collides with any of the objects. <br>
   * The last for loop is used to traverse through the paddles and determine their next location. <br>
   * if (s.getEarliestCollisionResponse().getTime() < tMin) - Checks if the earliest collision is within the millisecond of time.
   * 
   * @param float time the time of a single update.
   * @param Ball s the ball being checked for collisions.
   * @param ObstacleLine l the line being checked for collisions.
   * @param PseudoPaddle p the paddle being checked for collisions.
   * @param ButtonObstacle the button being checked for collisions.
   */
  ...
  /**
    ...
    * to infinity.
    * The result is passed back in the given CollisionResponse object. <br>
    * Written by Hock-Chuan Chau modified by Jason P'ng <br>
    * 
    * if (speedX == 0) - prevents arithmetic errors. Collision doesn't occur at a speed of 0. <br>
    * if (lineX > pointX) - determines which side of the line the ball is on. <br>
    * (t > 0 && t <= timeLimit) - Checks if a collision will occur withing the time limit. <br>
    * 
    * @param Ball s : The ball being checked for collision
    ...
    * @param distance : the distance from the line to the ball.
    * @param t : the time it takes for the ball to collide with the line with the current velocity.
    */
    ...
    * on the line (lambda), for a moving point and a line. <br>
    * Written by Hock-Chuan Chau modified by Jason P'ng <br>
    * 
    * @param Ball s : The ball being checked for collision
    * @param pointX : x-position of the center of the point.
  ...
  /**
    ...
    * Store and return the result in the given CollisionResponse object. <br>
    * Written by Hock-Chuan Chau modified by Jason P'ng <br>
    * 
    * @param Ball s : The ball being checked for collision
    * @param pointX : x-position of the center of the point.
